...
def get_pj_collision_manager(grasp_width, transform):
    """
    Based on grasp width, return different collision manager
    for franka gripper. Maximum grasp width is 5 cm.
    """
    TRUE_GRIPPER_MAX_WIDTH = 5
    grasp_width = TRUE_GRIPPER_MAX_WIDTH - grasp_width
    # hand
    hand_mesh = trimesh.load(
        "./models/Gripper/hande/robotiq_hande_description/meshes/hande.stl"
    )
    hand_mesh = hand_mesh.apply_scale(100)  # convert m to cm
    # finger left
    finger_left_mesh = trimesh.load(
        "./models/Gripper/hande/robotiq_hande_description/meshes/finger.stl"
    )
    finger_left_mesh = finger_left_mesh.apply_scale(100)
    # finger right
    finger_right_mesh = trimesh.load(
        "./models/Gripper/hande/robotiq_hande_description/meshes/finger.stl"
    )
    finger_right_mesh = finger_right_mesh.apply_scale(100)

    # create collsision manager for franka hand
    hande_collision_manager = trimesh.collision.CollisionManager()
    # add hand
    hand_trans = [
        [0, -1, 0, 0],
        [0, 0, -1, 0],
    ...
    hand_mesh.apply_transform(hand_trans_world)
    hande_collision_manager.add_object(name="hand", mesh=hand_mesh)
    ...
    finger_left_mesh.apply_transform(finger_left_trans_world_)

    hande_collision_manager.add_object(
        name="finger_left",
    ...
    finger_right_mesh.apply_transform(finger_right_trans_world)
    hande_collision_manager.add_object(
        name="finger_right",
        mesh=finger_right_mesh,
    )

    return hande_collision_manager
...
def sample_antipodal_grasps_geometrically(
    mesh, contact_points, contact_indexes, hdf5_file, mesh_name="", **config
):
    """
    Description:
    ...
    dset_grasp_transform = paralleljaw_subgrp.create_dataset(
        name="pregrasp_transform", shape=(max_grasps, 10), dtype="f"
    )

    dset_grasp_score = paralleljaw_subgrp.create_dataset(
        name="quality_score", shape=(max_grasps,), dtype="f"
    )

    # initialize collision manager
    collision_manager = trimesh.collision.CollisionManager()
    collision_manager.add_object(
        name="mesh_object", mesh=mesh, transform=np.identity(4)
    )
    ...
    for i in range(len(contact_points)):
        ...
        for k in range(tries):
            approach_vec_, center_ = generate_random_approach(
                max_rotation=max_angle_sampling, max_translation=max_translation
            )
            # get surface normal
            surface_norm_ = mesh.face_normals[contact_indexes[i]]
            # get transformation matrix between z-axis and surface normal
            H, _ = trimesh.geometry.align_vectors(surface_norm_, [0, 0, 1], True)
            ...
            center_pregrasp = (
                center_rot + contact_points[i] + (-1) * approach_vec_rot * (1.0)
            )
            faces_idx, _, pts_projected_ = trimesh.ray.ray_pyembree.RayMeshIntersector(
                mesh
            ).intersects_id(
            ...
            if len(pts_projected_) < 2:
                ...
                if v_l2 < max_gripper_length:
                    ...
                    if (
                        p1_angle <= max_angle_friction
                        and p2_angle <= max_angle_friction
                    ):
        ...
        if len(successful_candidates_) >= 1:
            ...
            for succ_idx in range(len(successful_candidates_) - 1):
                ...
                for _ in range(max_collision_tryouts):
                    ...
                    a_ = [
                        alphax,
                        alphay,
                        v_[0] / (v_[2] + 1e-12) * (-alphax)
                        + v_[1] / (v_[2] + 1e-12) * (-alphay),
                    ]
                    ...
                    grasp_width = min(
                        (w_ + PREGRASP_PARALLEL_DISTANCE), GRIPPER_WIDTH_MAX
                    )
                    gripper_collision_mesh = get_pj_collision_manager(
                        grasp_width=grasp_width, transform=np.array(grasp_transform_)
                    )
            ...
            pass
        print(
            "PJ | mesh {} | pt {:04} | # grasps {:05} | \
               antipodal curr. score {:03} ".format(
                mesh_name, i, counter, pos / tries
            )
        )
    return counter, grasps_config


def sample_suction_grasps_custom_model(
    mesh, contact_points, contact_indexes, hdf5_file, mesh_name="", **config
):
    """
    Description:
    ...
    max_grasps = config["max_grasps"]
    length_gripper = config["suction"][
        "length_gripper"
    ]  # length of gripper for collison
    radius_gripper = config["suction"][
        "radius_gripper"
    ]  # radius of gripper for collision
    # custom parameters
    r = config["suction"]["cup_radius"]  # radius of the suction cup
    h = config["suction"][
        "compensable_height_difference"
    ]  # compensable height difference
    num_mass_points = config["suction"][
        "num_mass_points"
    ]  # num of mass points for spring model
    ...
    dset_grasp_transform = suctioncup_subgrp.require_dataset(
        name="pregrasp_transform", shape=(max_grasps, 6), dtype="f"
    )
    ...
    suction_mesh = get_sc_collission_mesh(radius=radius_gripper, height=length_gripper)
    counter = 0
    ...
    for i in range(len(contact_points)):
        ...
        if mean_binary_score > 0.0:
            ...
            alphax = random.uniform(-1.0, 1.0)
            alphay = random.uniform(-1.0, 1.0)
            a_ = [alphax, alphay, v_[0] / v_[2] * (-alphax) + v_[1] / v_[2] * (-alphay)]
            a_ = a_ / np.linalg.norm(a_)
            c_ = np.cross(v_, a_)
            R_ = [[a_[0], c_[0], v_[0]], [a_[1], c_[1], v_[1]], [a_[2], c_[2], v_[2]]]
            pre_grasp_transform = [
                [R_[0][0], R_[0][1], R_[0][2], t_[0]],
                [R_[1][0], R_[1][1], R_[1][2], t_[1]],
                [R_[2][0], R_[2][1], R_[2][2], t_[2]],
            ...
            if colliding is False:
                ...
                grasps_config.append(
                    [contact_point, approach_vector, mean_binary_score]
                )
                counter += 1
            else:
        ...
        print(
            "SC | mesh {} | pt {:04} | # grasps {:05} | binary_score {} ".format(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mesh_root", default="./models/models_ifl", help="Root directory for meshes."
    )
    parser.add_argument(
        "--categories",
        type=str,
    ...
    parser.add_argument(
        "--com", action="store_true", help="Samle points based on Center of Mass."
    )
    parser.add_argument(
        "--paralleljaw",
        action="store_true",
    ...
    if args.categories is not None:
        for cat in category:
            obj_paths.append(
                glob.glob(
                    os.path.join(args.mesh_root, cat, "**/textured.obj"), recursive=True
                )[0]
            )
    else:
        obj_paths = glob.glob(
            os.path.join(args.mesh_root, "**/textured.obj"), recursive=True
        )
