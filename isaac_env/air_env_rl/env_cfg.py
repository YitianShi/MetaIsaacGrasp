...
def rgb_capture(env):
    ...
    rgb = rgb / 255.0
    imagenet_mean = torch.tensor([0.485, 0.456, 0.406], device=rgb.device).view(1, 1, 1, 1, 3)
    imagenet_std = torch.tensor([0.229, 0.224, 0.225], device=rgb.device).view(1, 1, 1, 1, 3)
...
def get_obj_height(env):
    ...
    objs_hight = torch.stack(
        [env.scene[f"obj_{i}"].data.root_pos_w[:, 2] for i in range(env.num_objs)]).T
...
def reset_root_state_sphere(
    ...
    SPHERE_RADIUS: float,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
    angle_min = 0, angle_max = 65 * pi / 180
):
    ...
    * It samples a random orientation and sets it into the physics simulation.
    ...
    if len(env_ids):
        center_pt = torch.tensor((desk_center[0], desk_center[1], 0.3), dtype=torch.float32)
...
class EventCfg:
    def __init__(self):
        ...
        for ix in range(N_MULTIPLE_CAM):
            setattr(
                self,
...
def grasp_success_compute(env) -> torch.Tensor:
    ...
    objs_hight = torch.stack([env._get_obj_pos(i)[:, -1] for i in range(NUM_OBJS)]).T # (batch_size, num_objs)
    # The obj is lifted if its height is above 90% of the lift height
    # The environment should reach the terminate state
    objs_hight_over_limit = objs_hight > LIFT_HEIGHT * 0.6 # (batch_size, num_objs) T/F
    success_env = torch.any(objs_hight_over_limit, dim=1) & (STATE_MACHINE["execute"] == env.sm_state) # (batch_size,)
    ...
    # if success_obj.numel() == 0:
    #     print("Warning: success_obj is empty! No object was lifted.")
    #     print("objs_hight_over_limit:", objs_hight_over_limit)
        
    if len(success_obj) > 0 and success_env.any():
        env.obj_chosen = torch.full((env.num_envs,), -1, dtype=torch.int64, device=env.device)
        env.obj_chosen[success_obj[:, 0]] = success_obj[:, 1]
        print("Chosen objects:", env.obj_chosen)

    return reward


def gripper_finger_capture(env):
    """Get the gripper open/close as observation"""
    ob_finger_state = env.scene[ROBOT_NAME].data.joint_pos[:,-2:]
    return ob_finger_state


def get_obj_position(env):
    # Assume only one obj!
    root_pos = env.scene[ROBOT_NAME].data.root_pos_w[:, :3]
    objs_position = env.scene["objs"].data.object_com_pos_w.clone().squeeze(1)
    return objs_position - root_pos
...
class ObservationsCfg:
    ...
    class ImageCfg(ObsGroup):
        ...
        def __init__(self):
            ...
            self.finger_state = ObsTerm(func=gripper_finger_capture)
            self.obj_position = ObsTerm(func=get_obj_position)
...
def gripper_distance_reward(env) -> torch.Tensor:
    ...
    # Get the positions of all objects (batch_size, num_objs, 3)
    objs_pos = torch.stack([env._get_obj_pos(i) for i in range(NUM_OBJS)], dim=1)
    # Get gripper position (batch_size, 3)
    gripper_pos = env._get_ee_pose()[:, :3]
    # Calculate the distance from the gripper to each object
    dist_to_objs = torch.norm(objs_pos - gripper_pos[:, None, :], dim=-1)
    min_distance, _ = torch.min(dist_to_objs, dim=1) # It may not be suitable when there are multiple objects.
    # Calculate dense reward
    alpha = 8.0
    reward = torch.exp(-alpha * min_distance) # - beta * min_distance
    return reward


def obj_height_reward(env) -> torch.Tensor:
    """(dense)Reward the agent when the object is lifted."""
    objs_pos = torch.stack([env._get_obj_pos(i) for i in range(NUM_OBJS)], dim=1) # (batch_size, num_objs, 3)
    # Assum only one obj!
    height = objs_pos[:, 0, 2] # (batchsize, num_objs)
    gripper_pos = env._get_ee_pose()[:, :3] # (batch_size, 3)
    dist_to_objs = torch.norm(objs_pos - gripper_pos[:, None, :], dim=-1).squeeze(-1)# (batchsize, num_objs)

    condition1 = (height > 0) & (height < 0.4) # (batchsize, num_objs)
    condition2 = dist_to_objs < 0.1 # (batchsize, num_objs)
    condition3 = gripper_pos[:, 2] > 0 # (batchsize,)
    condition4 = (STATE_MACHINE["execute"] == env.sm_state) # (batchsize,)
    reward = torch.where(condition1 & condition2 & condition3 & condition4, height, torch.zeros_like(height))

    return reward


def time_penalty(env) -> torch.Tensor:
    condition = (STATE_MACHINE["execute"] == env.sm_state) # (batchsize,)
    reward = torch.full((env.num_envs,), 0.05, device='cuda:0')
...
class RewardsCfg:
    ...
    grasp_success = RewTerm(func=grasp_success_compute, weight=100)
    gripper_distance = RewTerm(func=gripper_distance_reward, weight=1)
    height_reward = RewTerm(func=obj_height_reward, weight=1000)
    time_penalty = RewTerm(func=time_penalty, weight=-1)
