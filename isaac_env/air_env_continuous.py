...
class AIR_RLTaskEnv(ManagerBasedRLEnv):
    def __init__(self, cfg: CellEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.frame_wait_time_con_wp = wp.from_torch(
            self.frame_wait_time_con, wp.float32
        )
    ...
    def step(self, grasp_pose, policy_inference_criteria=torch.tensor([])):
        ...
        self.grasp_pose, self.gripper_state_con = (
                grasp_pose[:, :7],
                grasp_pose[:, -1],
            )
        ...
        while not policy_inference_criteria.any():
            ...
            policy_inference_criteria = self.advance_frame_con.clone()
        ...
        for env_id in ids:
            ...
            for cam_id in range(n_multiple_cam):
                ...
                if read_from_hdf5:
                    ...
                    if grasp_pos_img is not None and normal is not None:
                        ...
                        grasp_pos = self._get_obj_pos(self.obj_chosen[env_id])[env_id].squeeze(0)
    ...
    def save_data(
        ...
        if pcds is not None:
            ...
            for cam_id in range(n_multiple_cam):
                ...
                cv2.imwrite(f'{IMG_PATH}/env_{env_id}_epi_{episode}_step_{step}_camera_{cam_id}_frame_{time.time()}_rgb.png', rgb)
        ...
        print(f"{IMG_PATH}/env_{env_id}_epi_{episode}_step_{step}_frame_{time.time()}: Saved data")
